...
const App: React.FC = () => {
  ...
  // Load Initial Data from Local & Cloud
  useEffect(() => {
  ...
  const handleSaveUrl = (url: string) => {
    setBackendUrl(url);
    localStorage.setItem('angkringan_url', url);
  ...
  // Helper function to render pages conditionally
  const renderPage = () => {
    switch (activePage) {
      case 'Dashboard':
        return <Dashboard transactions={transactions} />;
      case 'POS':
        return <POS menuItems={menuItems} onCheckout={addTransaction} />;
      case 'History':
        return <TransactionHistory transactions={transactions} />;
      case 'Menu':
        return <MenuManagement menuItems={menuItems} onUpdateMenu={updateMenu} />;
      case 'Reports':
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-gray-50 overflow-hidden">
      ...
      <header className="lg:hidden sticky top-0 z-50 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <button onClick={() => setIsSidebarOpen(true)} className="p-2 hover:bg-gray-100 rounded-xl text-gray-700">
            <MenuIcon size={24} />
          </button>
        ...
        <div className="flex items-center gap-2">
          {backendUrl ? (
            <button onClick={fetchAllData} className={`p-2 rounded-xl ${isSyncing ? 'animate-spin text-blue-600' : 'text-green-500'}`}>
              <RefreshCw size={20} />
            </button>
          ) : <CloudOff size={20} className="text-gray-300" />}
        </div>
      </header>
      ...
      {/* Sidebar Drawer */}
      <aside className={`fixed inset-y-0 left-0 z-[70] w-72 bg-white border-r border-gray-100 transform transition-transform duration-300 ease-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 lg:w-64`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 mt-auto border-t border-gray-50 space-y-2">
            {!backendUrl && (
              <button onClick={() => setShowSettings(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-amber-50 text-amber-600 font-bold text-xs">
            ...
            {backendUrl && (
              <div className="flex items-center justify-between px-4 py-2 bg-green-50 rounded-xl mb-2">
                <div className="flex items-center gap-2 text-[10px] font-bold text-green-600">
                  <CloudCheck size={14} /> Terhubung Cloud
                </div>
                <button onClick={() => setShowSettings(true)} className="text-gray-400 hover:text-blue-600"><Settings size={14} /></button>
              </div>
            )}
      ...
      {/* Settings Modal (Cloud Setup) */}
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <h3 className="text-xl font-bold mb-2">Setup Cloud Backend</h3>
            <p className="text-sm text-gray-500 mb-6">Tempelkan URL Web App dari Google Apps Script Anda untuk sinkronisasi data.</p>
            
            <input 
              type="text" 
              defaultValue={backendUrl}
              placeholder="https://script.google.com/macros/s/.../exec"
              className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-mono text-xs mb-6"
              id="urlInput"
            />

            <div className="flex gap-3">
              <button onClick={() => setShowSettings(false)} className="flex-1 py-3 bg-gray-100 rounded-2xl font-bold text-gray-500">Batal</button>
              <button 
                onClick={() => {
                  const val = (document.getElementById('urlInput') as HTMLInputElement).value;
                  handleSaveUrl(val);
                }} 
                className="flex-1 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-100"
              >
                Simpan & Hubungkan
              </button>
            </div>
